...
void ParamWriter_DWDW::writeDoubleWishBone_Front(
    tx_car::MBD_DoubleWishBoneSusp &dw, int flip) {
  // spindle
  {
  ...
  // upper control arm
  {
    ...
    arm->mutable_offsetinchassisfront()->set_z(UCA_Z_ConnectorChassis);

    arm->mutable_offsetinchassisback()->set_x(-0.168);
    arm->mutable_offsetinchassisback()->set_y(0.446 * flip);
    arm->mutable_offsetinchassisback()->set_z(UCA_Z_ConnectorChassis);
  ...
  // lower control arm
  {
    ...
    arm->mutable_offset()->set_z(LCA_Z_ConnectorChassis * 0.5 - 0.118 * 0.5);
    ...
    arm->mutable_offsetinchassisfront()->set_z(LCA_Z_ConnectorChassis);

    arm->mutable_offsetinchassisback()->set_x(-0.223);
    arm->mutable_offsetinchassisback()->set_y(0.307 * flip);
    arm->mutable_offsetinchassisback()->set_z(LCA_Z_ConnectorChassis);
...
void ParamWriter_DWDW::writeDoubleWishBone_Rear(
    tx_car::MBD_DoubleWishBoneSusp &dw, int flip) {
  // spindle
  {
    auto spindle = dw.mutable_spindle();
  ...
  // damper
  {
    ...
    damper->mutable_offsetinchassis()->set_x(-0.104);
    damper->mutable_offsetinchassis()->set_y(0.6 * flip);
    damper->mutable_offsetinchassis()->set_z(0.256);

    damper->mutable_offsetinaxle()->set_x(-0.097);
    damper->mutable_offsetinaxle()->set_y(0.9 * flip);
    damper->mutable_offsetinaxle()->set_z(-0.047);
  }
}
...
void ParamWriter_DWDW::writeWheels(tx_car::MBD_Vehicle_DW_DW &vehicle) {
  ...
  auto setWheelInfo = [](tx_car::MBD_Wheel &wheelOut, bool isLeft,
                         bool isSteerable, int flip) {
    ...
    wheelOut.mutable_offsetinaxle()->set_y((0.91 + 0.22 / 2 + 0.06 / 2 + 0.03) *
                                           flip);
